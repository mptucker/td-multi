...
const dateLabel = (iso: string) => new Date(`${iso}T12:00:00Z`).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" });
  ...
  return (
    <main className="container py-16">
      ...
      <ul className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {Object.values(BRANDS).toSorted((a, b) => BRAND_UPDATED[b.slug].localeCompare(BRAND_UPDATED[a.slug])).map((b) => (
          <li key={b.slug} className="card p-5">
            <a href={`/?brand=${b.slug}`} className="flex h-full flex-col">
              ...
              <p className="mt-4 border-t border-black/10 pt-3 text-xs font-semibold uppercase tracking-wide text-muted">
                Updated <time dateTime={BRAND_UPDATED[b.slug]}>{dateLabel(BRAND_UPDATED[b.slug])}</time>
              </p>
            </a>
          </li>
        ))}
